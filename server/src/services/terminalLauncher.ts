...
function normalizeRuntime(runtime?: string): AiCliRuntime {
  if (runtime === 'openclaude' || runtime === 'codex' || runtime === 'gemini' || runtime === 'opencode') return runtime;
  const configured = getSettings().aiCliRuntime;
  if (configured === 'openclaude' || configured === 'codex' || configured === 'gemini' || configured === 'opencode') return configured;
  return 'openclaude';
}
